...
WAVE_1_GROUPS = [
    ({P.ПАСПОРТ}, "описываем сквозные процессы",
     "Паспорта блоков 1, 2 и 4: границы, вход, выход и показатели"),
    ({P.ПРОЕКТ}, "фиксируем границу между блоками",
     "Чек-лист передачи: общий выход реализации и вход сопровождения"),
...
ROADMAP = [
    ...
    ("Фаза 1", "Фундамент", "месяцы 2–3", "Паспорта 1, 2, 4 и чек-лист передачи"),
    ("Фаза 2", "Исполнение", "месяцы 3–6", "Двенадцать процедур возвращены в работу"),
    ("Фаза 3", "Проектирование", "месяцы 5–8", "Три новых регламента и SLA со смежниками"),
...
def build_headline(prs: Presentation) -> None:
    ...
    accent_bar(
        ...
        ["Первыми берём не самые выгодные процессы, а те, что задают точку отсчёта.",
         "Владение закреплено у каждого объекта. Пока нет паспорта процесса и замера «до», "
         "выгоду от оптимизации нечем доказать."],
    ...
    stats = [
        ...
        (str(BY_WAVE[P.В1]),
         f"{plural(BY_WAVE[P.В1], 'объект', 'объекта', 'объектов')} волны 1 задают "
         "точку отсчёта: паспорт и замер", BLUE),
...
def build_queue(prs: Presentation) -> None:
    ...
    title(slide, "Начало очереди: первые десять объектов",
          "Волна 1 целиком — паспорта процессов и базовая линия")
...
def build_top_value(prs: Presentation) -> None:
    ...
    accent_bar(
        ...
        ["Это вход проекта: приём пакета от ДРБ, приоритизация платежей, закупки "
         ...
         f"Они ждут волн {' и '.join(waves_used)}: сначала паспорт процесса и замер «до», "
...
def build_first_steps(prs: Presentation) -> None:
    ...
    title(slide, "Программа старта: пять первых действий",
          "Шаги 1–5 задают точку отсчёта: паспорта процессов и базовая линия")
